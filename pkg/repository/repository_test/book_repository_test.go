...
func TestBookRepository_GetAllBooksByFilter(t *testing.T) {
	...
	mock.ExpectQuery(`^SELECT b.id, b.isbn, b.name, b.publish_year, a.id, a.name, a.birth_day FROM book b JOIN author a ON b.author_id = a.id WHERE b.isbn = '1234567890' AND a.name = 'Author 1' AND b.publish_year >= 2011 AND b.publish_year <= 2012`).
		WillReturnRows(rows)
...
func TestBookRepository_UpdateBookById(t *testing.T) {
	...
	mock.ExpectExec("^WITH new_author AS \\( INSERT INTO author\\(name\\) VALUES \\(\\$1\\) RETURNING id::int \\) UPDATE book b SET name = 'Updated book' WHERE b.id = \\$2").
		WithArgs("", 2).
